...
func greedyOmNom(processID uint32, lunch chan backend.ProcessStream, err error) (uint32, error) {
	if err == nil {
		go func() {
			for _ = range lunch {
				// pretend we're an infinitely efficient client
				// so that writing stuff doesn't block writing to the consumer
				// see #65601002
			}
		}()
	}
...
var _ = Describe("Running processes", func() {
	...
	It("runs the command asynchronously via iomux-spawn", func() {
		...
		processID, err := greedyOmNom(processTracker.Run(cmd))
		...
		Eventually(fakeRunner).Should(HaveStartedExecuting(
			fake_command_runner.CommandSpec{
				Path: binPath("iomux-spawn"),
				Args: []string{
					fmt.Sprintf("/depot/some-id/jobs/%d", processID),
					"/bin/bash",
				},
				Stdin: "echo hi",
			},
	...
	It("returns a unique process ID", func() {
		...
		processID1, err := greedyOmNom(processTracker.Run(exec.Command("xxx")))
		Expect(err).NotTo(HaveOccurred())
		processID2, err := greedyOmNom(processTracker.Run(exec.Command("xxx")))
		Expect(err).NotTo(HaveOccurred())
	...
	It("creates the process's working directory", func() {
		...
		processID, err := greedyOmNom(processTracker.Run(exec.Command("xxx")))
		Expect(err).NotTo(HaveOccurred())
	...
	Context("when spawning fails", func() {
		...
		It("returns the error", func() {
			_, err := greedyOmNom(processTracker.Run(exec.Command("xxx")))
